...
public class BasicsLesson {
   ...
   case "ИЮНЬ":
    ...
    public Object newClass() {
        Object o = null;
        o=new LethalWeapon("green",1.34,134);
        return o;
    }
    ...
    public String stringExercise3(String input) {
        ...
        int pHello = input.indexOf("Hello");
        int pGoodbye = input.indexOf("Goodbye");
        // 1
        if (words.length == 3 && pHello >= 0) {
            result = words[0].trim() + " " + words[2].trim();
            return result;
        }
        // 2
        if (words.length == 2 && pHello >= 0) {
            result = new StringBuilder(input).reverse().toString();
            return result;
        }
        // 3
        if (words.length == 3 && pGoodbye >= 0) {
            result = words[0].trim() + " " + words[2].trim();
